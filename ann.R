...
pacman::p_load(tidyverse, readxl, zoo, neuralnet ,knitr,lubridate,tidymodels)

exchangeUSD <- read_excel("C:\\Users\\Hikam\\Desktop\\Machine Learning Module\\CW\\ExchangeUSD.xlsx") %>%
  janitor::clean_names() %>%
...
usd_exchange_full %>%
  pivot_longer(cols = 6:9,names_to = "kind",values_to = "rate") %>%
  ggplot(aes(date_in_ymd,rate, color = kind)) +
  geom_line() +
  facet_wrap(~kind) + theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=1
  )) +
...
normalized_usd = usd_exchange_full %>%
  mutate(across(3:13, ~normalize(.x)))
...
table(set_a_models_two_layers[4:6,])
